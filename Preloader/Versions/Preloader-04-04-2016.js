...
//
// last edit: 04-04-2016 by Jeremy
//
// List of changes:
//   - Now preloads only items in the sequence.
//
...
//     ##!! OBSOLETE !!##
//
// Allows you to preload images and audio files at the beginning
...
// Gets a list of the items in the sequence whose labels satisfy shuffleSequence
function getPredicatedItems(shuffleSequence, items) {
    ...
    var listOfLabelPredicates = getListOfLabelPredicates(shuffleSequence),
        listOfPredicatedItems = []; 

    // Going through the items passed to the function
    for (item in items) { 
        // 'concerned' indicates whether the item matches one of the predicates
        var concerned = false, label = items[item][0];
        ...
        for (pred in listOfLabelPredicates) {
            if (listOfLabelPredicates[pred](label)) concerned = true;
        }
        // If the label matched on the predicates, add the item to the array
        if (concerned) listOfPredicatedItems.push(items[item]);
    }
    
    // Return the list of label-matching items
    return listOfPredicatedItems;
  ...
  if (Object.keys(listOfFiles).length == 0) {
      ...
      for (item in predicatedItems) {
          ...
          if (Array.isArray(predicatedItems[item][0])) {
            ...
            if ($.inArray(""+group, Object.keys(groupsRanks)) < 0) groupsRanks[""+group] = 0;
          ...
          if (group == undefined || groupsRanks[group] == __counter_value_from_server__ % groupsLengths[group]) {
            for (element in predicatedItems[item]) {
              if (predicatedItems[item][element] == "Preloader") {
                  // console.log("Loading from item "+item+"."); // DEBUG
                  // console.log(predicatedItems[item]);         // DEBUG
                  // Retrieving the host for the pictures
                  hostURL = predicatedItems[item][parseInt(element)+1].host;
                  console.log(hostURL);
                  ...
                  for (file in predicatedItems[item][parseInt(element)+1].files) listOfFiles[hostURL+predicatedItems[item][parseInt(element)+1].files[file]] = "toLoad";
              }
            }
          }
      }
