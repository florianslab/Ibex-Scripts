...
//
// last edit: 02-19-2016 by Jeremy
//
...
// Necessary just if no host is passed as a parameter to the controller
...
function loadFile() {
      ...
      for (item in items) {
          for (element in items[item]) {
              if (items[item][element] == "Preloader") {
                  for (file in items[item][parseInt(element)+1].files) listOfFiles[host+items[item][parseInt(element)+1].files[file]] = "toLoad";
              }
          }
      }
  }
  ...
  if (!found) {
    ...
    /*for (file in listOfFiles) {
      if (listOfFiles[file] == "pending") {
        found = true;
        console.log("Loading pending "+file);
        break;
      }
    }
  }
